...
export const mutateElement = <TElement extends Mutable<ExcalidrawElement>>(
  ...
  // Clamp puzzle piece dimensions to fixed sizes (prevent resizing)
  if (element.type === "puzzlePiece") {
    if (typeof updates.width !== "undefined") {
      (updates as any).width = PUZZLE_SIZES_PX.A.width;
    }
    if (typeof updates.height !== "undefined") {
      (updates as any).height = PUZZLE_SIZES_PX.A.height;
    }
  } else if (element.type === "puzzlePiece2") {
    if (typeof updates.width !== "undefined") {
      (updates as any).width = PUZZLE_SIZES_PX.B.width;
    }
    if (typeof updates.height !== "undefined") {
      (updates as any).height = PUZZLE_SIZES_PX.B.height;
    }
  } else if (element.type === "puzzlePiece3") {
    if (typeof updates.width !== "undefined") {
      (updates as any).width = PUZZLE_SIZES_PX.C.width;
    }
    if (typeof updates.height !== "undefined") {
      (updates as any).height = PUZZLE_SIZES_PX.C.height;
    }
  }
