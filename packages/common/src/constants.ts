...
// Puzzle piece fixed sizes
// Real-world dimensions in centimeters
export const PUZZLE_SIZES_CM = {
  A: { width: 8.0767, height: 5.1729 }, // puzzlePiece (variant A)
  B: { width: 8.6, height: 8.6 }, // puzzlePiece2 (variant B)
  C: { width: 3.9384, height: 5.173 }, // puzzlePiece3 (variant C)
} as const;

// Scale factor: 100 pixels per centimeter
// This gives reasonable canvas sizes for collaboration and rendering
export const PX_PER_CM = 100;

// Convert cm to px and round to integers for pixel-perfect rendering
export const PUZZLE_SIZES_PX = {
  A: {
    width: Math.round(PUZZLE_SIZES_CM.A.width * PX_PER_CM),
    height: Math.round(PUZZLE_SIZES_CM.A.height * PX_PER_CM),
  },
  B: {
    width: Math.round(PUZZLE_SIZES_CM.B.width * PX_PER_CM),
    height: Math.round(PUZZLE_SIZES_CM.B.height * PX_PER_CM),
  },
  C: {
    width: Math.round(PUZZLE_SIZES_CM.C.width * PX_PER_CM),
    height: Math.round(PUZZLE_SIZES_CM.C.height * PX_PER_CM),
  },
} as const;

/**
 * Get fixed pixel dimensions for a puzzle piece variant.
 * Puzzle pieces are non-resizable and must maintain exact dimensions
 * to fit together perfectly.
 */
export const getPuzzleSizePx = (
  variant: "A" | "B" | "C",
): { width: number; height: number } => {
  return PUZZLE_SIZES_PX[variant];
};
...
export const TOOL_TYPE = {
  ...
  puzzlePiece2: "puzzlePiece2",
  puzzlePiece3: "puzzlePiece3",
  arrow: "arrow",
  line: "line",
  freedraw: "freedraw",
  text: "text",
